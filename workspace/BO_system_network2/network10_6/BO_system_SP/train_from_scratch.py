...
config_ini.read(config_filename) #config_ini.read('config_train.ini', encoding='utf-8')
...
target_list = eval(config_ini.get('RUN', 'target_list')) # Example=[0, 2]
# target_list = [0, 2, 3, 4, 5, 6] # mag, spin, gap, d, Lattice, Curie
# For example, [6, 6] means two output neurons in output layer represents Curie temperature. Both two neurons will be predict Curie temperatures with different internal expressions.
...
coeff_norm_M = np.array(eval(config_ini.get('RUN', 'output_mean'))) # Default=
coeff_norm_D = np.array(eval(config_ini.get('RUN', 'output_deviation'))) # Default=
...
number_of_neurons_list = eval(config_ini.get('RUN', 'number_of_neurons_list'))
# For example, in the case of Default=[100, 100, 10], three hidden layers with 100, 100, 10 neurons in each layer. 10 neurons in the last layer (closest to the output layer).
l2_scale = float(config_ini.get('RUN', 'l2_scale')) # Default=0.001
# Penalty size of L2 regularization. Equal or larger than 0.0
...
save_model = eval(config_ini.get('RUN', 'save_model')) # Default=True
# save model file (.h5)
...
csv_save_name_prefix = str(config_ini.get('RUN', 'csv_save_name_prefix')) # Default="predicted_"
# Recommended to end with an underscore(_)
...
# 毎探索ごとにshuffleのseedを変えるようにする。
mix_random_seed = number_of_materials
...
for ensemble_ID in range(number_of_ensemble): # Loop for number_of_ensemble
    ...
    #with strategy.scope(): # parallelization of this simple model is inefficient...
    model = Sequential()
    
    model.add(Dense(number_of_neurons_list[0], input_dim=length_of_input_features, kernel_regularizer=l2(l2_scale), activation='selu', kernel_initializer='lecun_normal'))
    for layer_loop in range(how_many_hidden_layers - 1):
        model.add(Dense(number_of_neurons_list[layer_loop+1], kernel_regularizer=l2(l2_scale), activation='selu', kernel_initializer='lecun_normal'))
    model.add(Dense(number_of_targets, kernel_regularizer=l2(l2_scale), activation='selu', kernel_initializer='lecun_normal'))
